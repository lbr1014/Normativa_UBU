...
GUIDED_QUESTION_TEXTS = {
    ...
    "explain_section": "explica el apartado indicado del documento, aclarando su finalidad, requisitos y efectos prÃ¡cticos.",
    "amounts": "resume las cantidades econÃ³micas, presupuesto, valor estimado e importes relevantes.",
}
...
def build_guided_question(form: RAGDefaultQueryForm) -> str:
    ...
    if question_kind == "explain_section" and section:
        task = f"explica el apartado \"{section}\" del documento, aclarando su finalidad, requisitos y efectos prÃ¡cticos."
    return f"{scope}, {task}".strip()


def guided_document_scope(document_id: str) -> str:
    """Devuelve una descripciÃ³n legible del documento seleccionado."""
    if not document_id:
        return ""
    try:
...
def build_expediente_type_payload() -> dict[str, list[str]]:
    """Compatibilidad con el antiguo payload de tipos por expediente."""
    return {}
