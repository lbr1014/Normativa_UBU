...
Sin embargo, la aplicación se ejecuta en una IP privada de la Universidad de Burgos. Las direcciones IP privadas no son accesibles directamente desde Internet, ya que están destinadas a redes internas. Existen dos formas principales de asociar un dominio a estas IP:
\begin{itemize}
\tightlist
	\item Configurar el \eng{router}: se configura para que el dominio apunte a la IP pública del \eng{router}. Mediante técnicas NAT (\eng{Network Address Translation}) se redirige el tráfico hacia la IP privada del servidor. Con este método se consigue que la aplicación sea accesible desde Internet. 
	\item Editar el archivo \eng{hosts}: en este archivo debe añadirse la relación entre el dominio y la IP privada. Esta alternativa solo tiene efecto en el equipo donde se modifica el archivo.
\end{itemize}
