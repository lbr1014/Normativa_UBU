...
\begin{itemize}
\tightlist
	\item Desarrollar un sistema basado en Generación Aumentada por Recuperación (\textit{RAG}) que permita realizar consultas sobre licitaciones del sector público, enriqueciendo las respuestas mediante información documental relevante.
...
\begin{itemize}
\tightlist
	\item RF-1 Gestión de documentos: el sistema debe permitir que el administrador gestione los documentos de licitaciones del sistema. 
	\begin{itemize}
	\tightlist
		\item RF-1.1 Inserción de documentos: el sistema debe permitir cargar documentos de licitaciones con el objetivo de construir y actualizar la base de datos vectorial.
		\item RF-1.2 Listado de documentos cargados: el sistema debe permitir ver los documentos cargados y sus metadatos.
		\item RF-1.3 Borrado de documentos: el sistema debe permitir borrar los documentos cargados.
		\item RF-1.4 Descarga de documentos: el sistema debe permitir descargar los documentos cargados.
	\end{itemize}
	\item RF-2 Procesado de los documentos: el sistema debe procesar de manera automática los documentos introducidos para prepararlos para que el \textit{RAG} pueda usarlos.
	\begin{itemize}
	\tightlist
		...
		\end{itemize}
	\end{itemize}
	\item RF-3 Segmentación en \textit{chunks}: el sistema debe dividir los documentos ya procesados en fragmentos de texto (\textit{chunks}) adecuados para el modelo de \textit{embeddings}.
	\begin{itemize}
	\tightlist
		\item RF-3.1 Tamaño: el sistema debe segmentar el texto respetando un máximo de tamaño medido en número de \textit{tokens} para evitar errores.
	...
	\begin{itemize}
	\tightlist
		\item RF-5.1 Almacenar \textit{chunks}: el sistema debe almacenar el contenido textual de cada \textit{chunk}.
		\item RF-5.2 Almacenar \textit{embeddings}: el sistema debe almacenar el \textit{embedding} asociado a cada \textit{chunk}.
		\item RF-5.3 Almacenar metadatos: el sistema debe almacenar metadatos relevantes como el nombre del archivo, el título y el segmento incluido en el \textit{chunk}.
		...
		\item RF-5.5 Eliminar: el sistema debe permitir borrar los \textit{chunks} y \textit{embeddings} de la base de datos vectorial.
	\end{itemize}
	...
	\end{itemize}
	\item RF-7 Construcción del \textit{prompt} con contexto: el sistema debe construir un \textit{prompt}, para alimentar al \textit{LLM}, que incluya los \textit{chunks} recuperados, la pregunta del usuario e instrucciones de uso del contexto.
	\item RF-8 Generación de respuestas mediante un \textit{LLM}: el sistema debe enviar el \textit{prompt} generado a un modelo \textit{LLM} y mostrar la respuesta de dicho modelo al usuario.
	...
	\tightlist
		...
		\item RF-10.5 Edición de datos de usuario: los usuarios deben poder editar sus datos.
	\end{itemize}
	...
	\tightlist
		...
		\item RF-11.2 Borrar consultas: los usuarios podrán borrar consultas del historial.
	\end{itemize}
	\item RF-12 Importación automática de licitaciones: el sistema debe permitir al administrador importar de manera automática las licitaciones mediante \textit{web scraping}.
	\begin{itemize}
	\tightlist
		\item RF-12.1 Descarga y almacenamiento: el sistema debe descargar y almacenar los documentos obtenidos, así como sus metadatos.
		\item RF-12.2 Gestión de errores y duplicados: el sistema debe continuar aunque la descarga de algún documento falle. Debe registrar los errores y evitar duplicados.
	\end{itemize}
\end{itemize}
...
\tightlist
	...
	\begin{itemize}
	\tightlist
		\item RNF-3.1 Documentos corruptos: el sistema debe saltarse los documentos corruptos o sin texto extraíble y continuar con el resto de documentos.
		\item RNF-3.2 \textit{Timeouts}: el sistema debe manejar los \textit{timeouts} del \textit{LLM} devolviendo mensajes controlados al usuario.
	\end{itemize}
	\item RNF-4 Trazabilidad técnica: el sistema debe realizar \textit{logs} de carga de modelo, lectura de los PDF, \textit{chunking}, \textit{embeddings}, guardado y consulta.
	...
	\begin{itemize}
	\tightlist
		\item RNF-5.1 Contraseñas: las contraseñas deben almacenarse encriptadas.
	...
	\item RNF-10 Evitar alucinaciones: el sistema no debe responder cuando no haya contexto relevante suficiente.
\end{itemize}
...
\tightlist
	...
	\begin{itemize}
	\tightlist
	 	\item \textbf{Usuario normal}: usuario autentificado que puede consultar el \textit{RAG} y ver su historial
...
\tightlist
	...
	\item CU-4 Realizar consulta al \textit{RAG}.
	\item CU-5 Consultar historial de consultas.
	\item CU-6 Borrar consultas del historial.
	\item CU-7 Gestión de documentos.
	\item CU-8 Cargar documentos de licitaciones.
	\item CU-9 Importar licitaciones automáticamente.
	\item CU-10 Convertir los documentos a \textit{Markdown}.
	\item CU-11 Indexar documento.
	\item CU-12 Listar documentos.
	\item CU-13 Consultar estado del documento.
	\item CU-14 Eliminar documento.
	\item CU-15 Descargar documento.
	\item CU-16 Gestión de usuario.
	\item CU-17 Crear usuario.
	\item CU-18 Borrar usuario.
	\item CU-19 Modificar rol del usuario.
	\item CU-20 Editar usuario.
\end{itemize}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			...
			\item El usuario introduce el nombre, \textit{email} y contraseña.
			\item El sistema valida el formato y que el \textit{email} no esté duplicado.
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Tiene algún campo vacío. Mostrar error de campo obligatorio.
			\item El \textit{email} ya existe. Mostrar error de \textit{email} duplicado.
			\item El \textit{email} no cumple la validación. Mostrar error de \textit{email} inválido.
			\item La contraseña no cumple la política. Mostrar error de contraseña poco segura.
			\item Las contraseñas no coinciden. Mostrar error de contraseñas distintas.
			\item El nombre tiene menos de 2 caracteres. Mostrar error de nombre demasiado corto.
		\end{enumerate}\\
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			...
			\item El usuario introduce el \textit{email} y la contraseña.
		...
		\begin{enumerate}
			...
			\tightlist
			\item Tiene algún campo vacío.  Mostrar error de campo obligatorio.
			\item El \textit{email} o la contraseña no coinciden con ningún usuario registrado. Mostrar error de usuario o contraseña incorrectos.
			\item El \textit{email} no cumple la validación. Mostrar error de \textit{email} inválido.
			\item La contraseña no cumple la política. Mostrar error de contraseña poco segura.		
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Error al invalidar sesión. Mostrar mensaje de error al cerrar sesión. 	
		\end{enumerate}\\
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-4}    & \textbf{Realizar consulta al \textit{RAG}}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-6, RF-7, RF-8, RF-9, RF-11, RNF-3, RNF-3.2, RNF-10 \\
		\textbf{Descripción}          & El usuario hace una pregunta, el sistema recupera contexto por similitud, construye el \textit{prompt} y genera respuesta que incluya las fuentes. \\
		...
		\begin{enumerate}
			...
			\item El sistema llama al \textit{LLM} y genera respuestas.
		...
		\begin{enumerate}
			...
			\item \textit{Timeout} del \textit{LLM}. Mostrar un mensaje controlado. 	
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-4 Realizar consulta al \textit{RAG}.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item El usuario no ha realizado consultas. Mostrar mensaje de que aun no hay consultas. 	
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Error en la base de datos al intentar borrar la consulta. Mostrar mensaje de error controlado.
			\item La consulta seleccionada no existe o ya ha sido borrada. Mostrar aviso al usuario.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-7}    & \textbf{ Gestión de documentos}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1, RF-1.1, RF-1.2, RF-1.3, RF-2, RF-2.1, RF-2.2, RF-2.2.1, RF-2.3, RF-2.3.1, RF-2.3.2, RF-3, RF-4, RF-5, RF-5.1, RF-5.2, RF-5.3, RF-5.4, RF-5.5, RNF-3.1 \\
		...
		\begin{enumerate}
			...
			\item El administrador introduce su \textit{email} y contraseña.
			\item El sistema le autentifica como administrador.
			\item El administrador accede a <<Administrar documentos>>.
		\end{enumerate}\\
		\textbf{Postcondición}        & Posibilidad de cargar documentos nuevos (de forma manual o por \textit{web scraping}), indexar, borrar o descargar los documentos ya cargados . \\
		\textbf{Excepciones}          & 
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Intento de acceso sin ser administrador. Se deniega.
			\item Si no hay documentos cargados en la aplicación. Muestra un mensaje.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-7 Gestión de documentos.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-8}    & \textbf{Cargar documentos de licitaciones}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1, RF-10.3, RF-10.4, RNF-3, RNF-3.1 \\
		...
		\bottomrule
	\end{tabularx}
	\caption{CU-8 Cargar documentos de licitaciones.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-9}    & \textbf{Importar licitaciones automáticamente}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-12, RF-12.1, RF-12.2, RNF-3, RNF-3.1\\
		\textbf{Descripción}          & Permite al administrador importar de manera automática las licitaciones a través de \textit{web scraping}. \\
		\textbf{Precondición}         & Que el administrador esté autentificado y haya un programa de \textit{web scraping} funcional. \\
		\textbf{Acciones}             &
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			...
			\item El sistema lanza el proceso de \textit{web scraping}.
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Fallo de red. Se registra y muestra al administrador.
			\item Documento corrupto o sin texto legible. Se marca como fallido y se continua con el siguiente.
			\item Documento duplicado. Se omite y se registra como duplicado.
		\end{enumerate}\\
		\textbf{Importancia}          & Media  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-9 Importar licitaciones automáticamente.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-10}    & \textbf{Convertir los documentos a \textit{Markdown}}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		...
		\end{enumerate}\\
		\textbf{Postcondición}        & Los documentos convertidos a \textit{Markdown} listos para el \textit{chunking}. \\
		\textbf{Excepciones}          & 
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
		...
		\bottomrule
	\end{tabularx}
	\caption{CU-10 Convertir los documentos a \textit{Markdown}.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-11}    & \textbf{Indexar documento}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
...
vectorial cuando hay documentos nuevos. El sistema convierte los documentos procesados en \textit{chunks}, genera los \textit{embeddings} y los guarda en la base de datos vectorial con metadatos. \\
		...
		\begin{enumerate}
			...
			\item Error de conexión con la base de datos vectorial. Se aborta la operación y se avisa al administrador
a través de un mensaje.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-11 Indexar documento.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-12}    & \textbf{Listar documentos}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1.2, RNF-4 \\
		...
		\begin{enumerate}
			...
			\tightlist
			\item El administrador accede a <<Administrar documentos>>.
			\item El sistema lista los documentos con sus metadatos (nombre, tamaño, fecha de modificación y \textit{chunks}).
		\end{enumerate}\\
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item No hay documentos. Muestra un mensaje informativo <<Aún no hay documentos>>.
			\item Error de base de datos. Muestra un error.
		\end{enumerate}\\
		\textbf{Importancia}          & Media  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-12 Listar documentos.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-13}    & \textbf{Consultar estado del documento}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1.2, RNF-4 \\
		...
		\bottomrule
	\end{tabularx}
	\caption{CU-13 Consultar estado del documento.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-14}    & \textbf{Eliminar documento}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1.3, RF-5.5, RNF-5.2 \\
		\textbf{Descripción}          & Permite al administrador eliminar un documento del sistema, así como sus \textit{chunks} y \textit{embeddings} asociados en la base de datos vectorial. \\
		\textbf{Precondición}         & Hay un administrador autentificado y existe al menos un documento cargado en el sistema. \\
		\textbf{Acciones}             &
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item El administrador accede a <<Administrar documentos>>.
			\item El sistema lista los documentos con sus metadatos (nombre, tamaño, fecha de modificación y \textit{chunks}) y estado.
			\item El administrador pulsa el botón <<Borrar>>.
			\item El sistema pide confirmación.
			\item El administrador confirma el borrado.
			\item El sistema borra el documento y sus metadatos. Además, si el documento esta indexado borra sus \textit{chunks} y sus \textit{embeddings}.
			\item El sistema actualiza el listado.
		\end{enumerate}\\
		\textbf{Postcondición}        & Documento eliminado y sus \textit{chunks} y \textit{embeddings} borrados de la base de datos vectorial. \\
		\textbf{Excepciones}          & 
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item El documento no existe. Se muestra un aviso. 
			\item Error al borra. No se borra y se informa al administrador.
			\item Error al borrar en la base de datos vectorial. No se borra el documento y se informa al administrador.
		\end{enumerate}\\
		\textbf{Importancia}          & Alta  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-14 Eliminar documento.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-15}    & \textbf{Descargar documento}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-1, RF-1.4, RNF-5.2 \\
		...
		\begin{enumerate}
			...
			\tightlist
			\item El documento no existe. Se muestra un aviso. 
			\item Error al descargar. Se informa al administrador.
		\end{enumerate}\\
		\textbf{Importancia}          & Media  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-15 Descargar documento.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-16}    & \textbf{ Gestión de usuarios}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		...
		\begin{enumerate}
			...
			\tightlist
			\item Intento de acceso sin ser administrador. Se deniega.
			\item Si no hay usuarios registrados en la aplicación. Mostrar mensaje de que aún no hay usuarios.
		\end{enumerate}\\
		\textbf{Importancia}          & Media  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-16 Gestión de usuarios.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-17}    & \textbf{ Crear usuario}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-10.3, RF-10.4 \\
		...
		\begin{enumerate}
			...
			\item El administrador introduce el nombre, \textit{email}, contraseña y si es administrador o no.
			\item El sistema valida el formato y que el \textit{email} no esté duplicado.
			\item El sistema almacena el usuario con la contraseña encriptada.
		...
		\begin{enumerate}
			...
			\item El \textit{email} ya existe. Muestra un error con el mensaje <<Ya existe un usuario con ese \textit{email}>>.
			\item El \textit{email} no cumple la validación. Muestra un error con el mensaje <<Invalid \textit{email} address>>.
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-18}    & \textbf{ Borrar usuario}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-10.3, RF-10.4 \\
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Fallo al conectar con la base de datos. No se borra el usuario y se informa del problema al administrador.
		\end{enumerate}\\
		\textbf{Importancia}          & Media  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-18 Borrar usuario.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-19}    & \textbf{ Modificar rol del usuario}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-10.3, RF-10.4 \\
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Fallo al conectar con la base de datos. No se cambia el rol del usuario y se informa del problema al administrador.
		\end{enumerate}\\
		\textbf{Importancia}          & Baja  \\
		\bottomrule
	\end{tabularx}
	\caption{CU-19 Modificar rol del usuario.}
\end{table}
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.21\columnwidth} p{0.71\columnwidth} }
		\toprule
		\textbf{CU-20}    & \textbf{ Editar usuario}\\
		\toprule
		\textbf{Versión}              & 1.0    \\
		\textbf{Autor}                & Lydia Blanco Ruiz \\
		\textbf{Requisitos asociados} & RF-10.5, RNF-5.1, RNF-5.2 \\
		...
		\begin{enumerate}
			...
			\item Modifica los campos que desee (nombre, \textit{email} o contraseña).
		...
		\begin{enumerate}
			\def\labelenumi{\arabic{enumi}.}
			\tightlist
			\item Se intenta cambiar el \textit{email} por otro que ya existe en el sistema. Muestra un error con el mensaje <<Ya existe un usuario con ese \textit{email}>>.
			\item El \textit{email} no cumple la validación. Muestra un error con el mensaje <<Invalid \textit{email} address>>.
			\item La contraseña no cumple la política. Muestra un error con el mensaje <<Aumenta la longitud del texto a 6 caracteres como mínimo>>.
			\item El nombre tiene menos de 2 caracteres. Muestra un error con el mensaje <<Aumenta la longitud del texto a 2 caracteres o más>>.
		\end{enumerate}\\
