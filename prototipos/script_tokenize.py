...
def clean_page(text: str) -> str:
    ...
    for ln in (text or "").splitlines():
        s = ln.strip()
        if not s: continue
        if RE_HEADER_FOOTER.match(s): continue
        if RE_INDEX_LINE.search(s): continue
        if RE_TRAILING_PAGENUM.search(s): continue
        out.append(s)
    return "\n".join(out)
    ...
    for ln in lines:
        if looks_like_heading(ln):
            ...
            if cur_title is None:
                # si el documento empieza con texto antes de un título, crea un “Preámbulo”
                cur_title = "Preámbulo"
            cur_buf.append(ln)
